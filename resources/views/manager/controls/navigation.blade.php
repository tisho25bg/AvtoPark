            ...
            <ul class="nav navbar-nav navbar-right">
                 <li class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li class="red">You have {{$customerOrders->count()}} new orders</li>
                        @if($customerOrders->count() > 0)
                            @foreach($customerOrders->get() as $order)
                                <li><a data-order="{{$order->id}}"  href="{{ route('new-orders') }}">{{$order->services->name}} - {{$order->customer->fullName}}</a></li>
                            @endforeach
                            <li><a href="{{route('manager')}}">For more  click here</a></li>
                        @endif
                    </ul>
                </li>
